...
REST framework also ships with an optional exception handler that formats error responses according to [RFC 9457, "Problem Details for HTTP APIs"][rfc9457]. Instead of the default `{"detail": ...}` body, handled errors are served with the `application/problem+json` media type and a standardized set of members.

The handler deals with exactly the same exceptions as the default handler (REST framework's `APIException`, together with Django's `Http404` and `PermissionDenied`), and returns `None` for anything it does not recognize. Enable it by pointing the `EXCEPTION_HANDLER` setting at the built-in handler:
...
* `type` - A URI reference that identifies the problem type. It defaults to `about:blank`, which indicates that the problem has no additional semantics beyond those of the HTTP status code.
* `title` - A short, human-readable summary of the problem. This matches the HTTP status phrase for the response status code (for example `Not Found` or `Too Many Requests`).
* `status` - The HTTP status code generated by the origin server, which is always identical to the response status code.
* `detail` - A human-readable explanation specific to this occurrence of the problem.
* `instance` - The relative URI of the request that produced the error, including any query string.

### Validation errors

Validation errors are never flattened into a single string. Every leaf error is reported through the `errors` extension member. Each entry contains:

* `pointer` - A [JSON Pointer (RFC 6901)][rfc6901] to the offending value in the request, preserving list indices. Reference tokens are escaped so that `~` becomes `~0` and `/` becomes `~1`. Errors that are not tied to a particular request field (for example a single, non-validation `APIException`) carry no `pointer`.
* `detail` - The human-readable error message.
* `code` - The original `ErrorDetail.code` of the error.

For example, a `400 Bad Request` validation failure might produce:

    {
        ...
        "instance": "/items/",
        "errors": [
            {"pointer": "/name/0", "detail": "This field is required.", "code": "required"},
            {"pointer": "/tags/1", "detail": "A valid integer is required.", "code": "invalid"},
            {"pointer": "/a~1b/0", "detail": "This field is required.", "code": "required"}
        ]
    }

Dictionaries, lists and `non_field_errors` at any level of the validation structure are preserved this way, rather than being coerced into a string.

### Customizing the problem type

Custom `APIException` subclasses can provide a stable problem type, title and additional extension members by setting the `problem_type`, `problem_title` and `problem_extensions` attributes:

    from rest_framework.exceptions import APIException

    class AccountLocked(APIException):
        status_code = 423
        default_detail = 'Your account is locked.'
        default_code = 'account_locked'
        problem_type = 'https://example.com/probs/account-locked'
        problem_title = 'Account Locked'
        problem_extensions = {'locked_until': '2024-01-01T00:00:00Z'}

Extension members are merged into the response object. The standard members (`type`, `title`, `status`, `detail` and `instance`) and the `errors` extension cannot be overridden by `problem_extensions`; any conflicting extension keys are ignored.

The status codes and the `WWW-Authenticate` (for `401 Unauthorized` responses) and `Retry-After` (for `429 Too Many Requests` throttling responses) headers are preserved exactly as with the default handler. When an HTML renderer such as the browsable API is negotiated, the usual HTML error page is rendered instead, so enabling this handler does not change content negotiation, successful responses, or the behavior of any other exception handler.
